...
if IsServer() then
    ...
    function modifier_centaur_custom_return:OnTakeDamage(keys)
        ...
        if unit == self:GetParent() and unit ~= attacker and not unit:HasModifier("modifier_centaur_custom_return_cooldown") then
            ...
            unit:AddNewModifier(caster, ability, "modifier_centaur_custom_return_cooldown", {
                duration = ability:GetSpecialValueFor("cooldown")
            })
        end
    end
end
