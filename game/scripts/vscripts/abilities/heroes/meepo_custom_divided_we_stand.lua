...
function meepo_copy_items(target, illusion)
    for itemSlot = 0, 5 do
        ...
        if item ~= nil and item:IsPassive() then
...
function create_clone(caster, ability)
    ...
    local illusion = CreateUnitByName(caster:GetUnitName(), spawnPos, true, caster, nil, caster:GetTeamNumber())
    ...
    illusion:AddNewModifier(illusion, ability, divided_we_stand_modifier, {})
...
function kill_alive_clones(caster)
    ...
    for i, unit in ipairs(ownedUnits) do
        if unit:IsAlive() and unit:HasModifier(divided_we_stand_modifier) then
            kill_illusion(unit)
        end
    end
end
...
function modifier_meepo_custom_divided_we_stand:GetMinHealth(keys)
    return 1
end


function modifier_meepo_custom_divided_we_stand:GetModifierDamageOutgoing_Percentage(keys)
    return -50
end
